...
## 变更
- 更新 `index.html`：在加载 `app.js` 前加载 `src/keys.js`、`src/utils.js`、`src/storage.js`、`src/i18n.js`、`src/effects.js` 与 `src/pools.js`。
- 更新 `app.js`：接入 `RememberKeys` / `RememberUtils` / `RememberStorage` / `RememberI18n` / `RememberEffects` / `RememberPools`，并将 key 生成、随机/洗牌、localStorage 读写、i18n、音效/震动、以及素材池/主题素材选择相关逻辑改为调用模块实现（保留原函数名以兼容现有调用与 Jest）。
- 更新 `src/storage.js`：补齐每日挑战完成状态、引导隐藏状态、以及“清空本地数据”的 key 批量删除支持。
- 更新 `sw.js`：预缓存列表加入 `src/keys.js`、`src/utils.js`、`src/storage.js`、`src/i18n.js`、`src/effects.js` 与 `src/pools.js`。
