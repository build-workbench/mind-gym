...
- 新增无构建模块：
  ...
  - `src/ui-events.js`：集中管理 UI 事件绑定（addEventListener 注册）与回调接线（UMD：浏览器/Node/Jest 兼容）。

## 变更
- 更新 `index.html`：在加载 `app.js` 前加载 `src/keys.js`、`src/utils.js`、`src/storage.js`、`src/i18n.js`、`src/effects.js`、`src/pools.js`、`src/timer.js`、`src/confetti.js`、`src/ui-events.js` 与 `src/ui.js`。
- 更新 `app.js`：接入 `RememberKeys` / `RememberUtils` / `RememberStorage` / `RememberI18n` / `RememberEffects` / `RememberPools` / `RememberTimer` / `RememberConfetti` / `RememberUIEvents` / `RememberUI`，并将 DOM 元素绑定与事件绑定逻辑改为调用模块实现（保留原变量/回调以兼容现有调用与 Jest）。
- 更新 `src/storage.js`：补齐每日挑战完成状态、引导隐藏状态、以及“清空本地数据”的 key 批量删除支持。
- 更新 `sw.js`：预缓存列表加入 `src/keys.js`、`src/utils.js`、`src/storage.js`、`src/i18n.js`、`src/effects.js`、`src/pools.js`、`src/timer.js`、`src/confetti.js`、`src/ui-events.js` 与 `src/ui.js`。
