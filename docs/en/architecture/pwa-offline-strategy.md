...
1. **Service Worker (`sw.js`)** for caching and update flow.
2. **Web App Manifest (`manifest.webmanifest`)** for installability and standalone launch.
3. **localStorage (`src/storage.js`)** for durable user-owned progress.
...
The project prefers a conservative update posture: fetch fresh shell content when possible, but keep cached content available so the app stays functional when the network is absent or slow.
